...
<body>
  ...
  if (isLogged() == 1) {
    ...
    echo "<div class='container products-tab-carousel'>
    <nav>
      <div class='nav nav-tabs' id='nav-tab' role='tablist'>
      <h5 style='display:block;color:#3D443A' class=' p-2'><b>Based on your last search</b></h5>
    ...
    mysqli_query($clink, "alter table advertisments add FULLTEXT(`Title`, `Details`) ");
    ...
    if (mysqli_num_rows($rec) > 0) {
      ...
      while ($row = mysqli_fetch_assoc($rec)) {
        echo "  <div class='item' data-product>
            ...
            <div class=''>
            <div class='text-center card card-body h-100 ' > 
            <span class=' priceTk float-left'> Tk {$row['Price']}</span>
              <a href='pageads.php?ADS-ID={$row['AdsID']}' data-url>
              <img style=' height: 200px;width: 150px;margin-left:-10px' class='' src='assets/img/{$row['Image']}' class='card-img-top img-fluid' alt='...'>
              </a>
            <div class='title2'>
            <h5 style='display:inline-block' class='card-title'>{$row['Title']}</h5>
              ...
              <div class='btngroup'>
                <a
                  ...
                  href='pageads.php?ADS-ID={$row['AdsID']}'
  ...
  if (isLogged() == 1) {
    echo "
    <a style='text-align:center;' href='viewmore.php'>
    <h5 style='display:block' class= 'view card-title'>
      <u>View more</u>
    </h5>
  </a>";
  }
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    <section class='ads'>
      <div class='container'>
        <h5 style=' display:block;color:#3D443A' class='card-title'><b>$filter products</b></h5>
        <hr>
        <div class='row '>
  ";
